        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
          {skills.map((skill, index) => (
            ...
            >
              ...
              {/* Card Content */}
              <div className="relative bg-white border-2 border-gray-200 rounded-2xl p-6 hover:shadow-xl transition-all duration-300">

                {/* Icon */}
                <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${skill.gradient} flex items-center justify-center mb-4 shadow-lg transform transition-transform group-hover:scale-110 group-hover:rotate-6`}>
                  <skill.icon className="h-7 w-7 text-white" />
                </div>

                {/* Title */}
                <h3 className="text-xl font-black text-gray-800 mb-2 group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:bg-clip-text group-hover:from-teal-600 group-hover:to-cyan-600 transition-all">
                  {skill.name}
                </h3>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-700 font-semibold">{skill.levelText}</span>
                  </div>
            ...
            <div className="flex gap-4 animate-scroll">
              {[...tools, ...tools].map((tool, index) => (
                ...
                >
                  ...
                  <div className="relative bg-white border-2 border-gray-200 rounded-2xl px-8 py-5 hover:border-teal-100 hover:shadow-lg transition-all">
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${tool.color} flex items-center justify-center shadow-md`}>
                        <Zap className="h-5 w-5 text-white" />
                      </div>
                      <div>
                        <p className="font-black text-gray-800 text-lg">{tool.name}</p>
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-gray-600 font-semibold">{tool.category}</span>
                          <span className="text-xs text-teal-600 font-bold">{tool.usage}%</span>
                        </div>
                      </div>
                    </div>
                  </div>
